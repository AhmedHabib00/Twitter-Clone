...
class TestNotification:
    ...
    def test_reply_tweet_notification(self):
        ...
        time.sleep(1)
        try:
            driver.find_element(By.ID, accessabilities.reply_text_box_id).send_keys('hi')
        except NoSuchElementException:
            pass
        time.sleep(1)

        try:
            driver.find_element(By.ID, accessabilities.whisper_reply_button_id).click()
        except NoSuchElementException:
            pass
        time.sleep(1)
        try:
            driver.find_element(By.CSS_SELECTOR, accessabilities.close_button).click()
        except NoSuchElementException:
            pass
        # sign out
        try:
