...
router.post("/",multer.any(),auth, async function(req,res,next){
    ...
    try
    {
        var userInfo=await user.findById(token.toString())     
    }
            ...
            //  replyTo:replyTemp
             ...
             else
             {
                ...
                //If it is not a reply
                 ...
                 pusher = notificationData();
                 numbFollowers = userInfo.followers.length
                 followersList = userInfo.followers;
                 console.log(numbFollowers)
                 for(var i = 0; i <= numbFollowers-1;i++)
                 {
                     console.log(followersList[i].toString());
                    pusher.trigger(followersList[i].toString(), "my-event", {//should be notification event for posting a new tweet
                        message: userInfo.username + " Has Posted a new tweet"
                      });
                 }
...
router.put("/:id/like",auth,async(req,res)=>{
    ...
    pusher = notificationData();
    pusher.trigger(postId.postedBy.toString(), "my-event", {//should be notification event for posting a new tweet
        message: userInfo.username + " Has liked your tweet"
        });
        return res.sendStatus(200);
